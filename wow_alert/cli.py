...
def build_app(config: AppConfig) -> tuple[QApplication, MainWindow]:
    ...
    spell_db = YamlSpellDb.from_yaml(
        config.spell_db_path,
        dungeon=config.dungeon,
        player_class=config.player_class,
        player_spec=config.player_spec,
    ...
    window = MainWindow(worker, alert_player, show_preview=config.show_preview)
    window.start()
    return app, window
